	...
	*/
	...
	switch ($_POST['LEVEL']) {
		...
		case 'DEPT':
			...
			$requete_supp = '';
			if (!empty($_POST['FILTRE']) && $_POST['ACTIVATION']=='1') {
				$requete_supp .= " AND com_nom LIKE '".$_POST['FILTRE']."%' ";
				$ajout_lib = '/FILTRE='.utf8_decode($_POST['FILTRE']);
			}
		   	$requete  = "SELECT com_id,com_nom
							 FROM insee_communes
								WHERE com_dep='".$_POST['ID']."'".$requete_supp."
									ORDER BY com_nom"; 
		    $base->bdd_execsql($requete);
		   	while($base->bdd_lire_ligne()) {
				UneLigne('VILLE',$base->bdd_lire_champ("com_id"),$base->bdd_lire_champ("com_nom").$ajout_lib );
			}
			break;
