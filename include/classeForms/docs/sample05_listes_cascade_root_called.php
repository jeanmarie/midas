<?php
	...
	/* page qui genere du XML au profit des pages : 
		- sample05_listes_cascade7.php
		la section ROOT permet d'initialiser la liste "racine" en ajax sans avoir
		recours a un pre-chargement de l'objet frm_ObjetListesCascade() au moyen du code :
			-----------------------------------------------
			$requete  = "SELECT * 
							FROM insee_regions
								ORDER BY reg_nom"; 	    
	...
	*/
	...
	switch ($_POST['LEVEL']) {
		// traitement du la liste ROOT ----------------------------
		case 'ROOT':
		   	$requete  = "SELECT reg_id,reg_nom
							 FROM insee_regions								
									ORDER BY reg_nom"; 
		    $base->bdd_execsql($requete);
